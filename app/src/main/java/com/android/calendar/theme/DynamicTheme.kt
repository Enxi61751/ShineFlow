...
private const val COLOR_THEME_PREF = "pref_color_theme"

// Resolve the selected ShineFlow dopamine palette to its ThemeOverlay style.
fun getColorThemeOverlay(context: Context): Int {
    return when (Utils.getSharedPreference(context, COLOR_THEME_PREF, "coral")) {
        "mango" -> R.style.ThemeOverlay_ShineFlow_Mango
        "grape" -> R.style.ThemeOverlay_ShineFlow_Grape
        "mint" -> R.style.ThemeOverlay_ShineFlow_Mint
        "sky" -> R.style.ThemeOverlay_ShineFlow_Sky
        "bubblegum" -> R.style.ThemeOverlay_ShineFlow_Bubblegum
        else -> R.style.ThemeOverlay_ShineFlow_Coral
    }
}
...
fun AppCompatActivity.applyTheme() {
    ...
    // Apply the selected ShineFlow dopamine color palette on top of the base theme.
    theme.applyStyle(getColorThemeOverlay(this), true)
