...
public class EditEventView implements View.OnClickListener, DialogInterface.OnCancelListener,
    ...
    // ShineFlow time-mode segmented control (time period / time point / all day)
    com.google.android.material.button.MaterialButtonToggleGroup mTimeModeGroup;
    // ShineFlow special all-day type selector (none / anniversary / birthday / countdown)
    com.google.android.material.button.MaterialButtonToggleGroup mSpecialDayGroup;
    static final int TIME_MODE_PERIOD = 0;
    static final int TIME_MODE_POINT = 1;
    ...
    private boolean fillModelFromUI() {
        ...
        // ShineFlow: special all-day types and time-point handling.
        if (mModel.mAllDay) {
            mModel.mSpecialType = mSpecialType;
            if ((mSpecialType == ExtendedProperty.EVENT_TYPE_BIRTHDAY
                    || mSpecialType == ExtendedProperty.EVENT_TYPE_ANNIVERSARY)
                    && mModification != EditEventHelper.MODIFY_SELECTED) {
                // Birthdays and anniversaries repeat every year.
                mModel.mRrule = "FREQ=YEARLY";
            }
        } else {
            mModel.mSpecialType = ExtendedProperty.EVENT_TYPE_NONE;
            if (mTimeMode == TIME_MODE_POINT) {
                // A time point has no duration: end equals start.
    ...
    private void setupTimeModeControls() {
        if (mTimeModeGroup == null) {
            return;
        }
        mTimeModeGroup.addOnButtonCheckedListener((group, checkedId, isChecked) -> {
            if (!isChecked || mUpdatingTimeMode) {
                return;
            }
            if (checkedId == R.id.btn_mode_point) {
                applyTimeMode(TIME_MODE_POINT);
            } else if (checkedId == R.id.btn_mode_allday) {
                applyTimeMode(TIME_MODE_ALL_DAY);
            } else {
                applyTimeMode(TIME_MODE_PERIOD);
            }
        });
        if (mSpecialDayGroup != null) {
            mSpecialDayGroup.addOnButtonCheckedListener((group, checkedId, isChecked) -> {
                if (!isChecked) {
                    return;
                }
                if (checkedId == R.id.btn_special_anniversary) {
                    mSpecialType = ExtendedProperty.EVENT_TYPE_ANNIVERSARY;
                } else if (checkedId == R.id.btn_special_birthday) {
                    mSpecialType = ExtendedProperty.EVENT_TYPE_BIRTHDAY;
                } else if (checkedId == R.id.btn_special_countdown) {
                    mSpecialType = ExtendedProperty.EVENT_TYPE_COUNTDOWN;
                } else {
                    mSpecialType = ExtendedProperty.EVENT_TYPE_NONE;
                }
            });
        }
    }

    // React to the user picking a time mode: drive the (hidden) all-day
    // checkbox and toggle the end date/time and special-day selector.
    private void applyTimeMode(int mode) {
        mTimeMode = mode;
        boolean allDay = (mode == TIME_MODE_ALL_DAY);
        boolean point = (mode == TIME_MODE_POINT);

        if (mAllDayCheckBox.isChecked() != allDay) {
            mAllDayCheckBox.setChecked(allDay); // triggers setAllDayViewsVisibility(...)
        } else {
            setAllDayViewsVisibility(allDay);
        }

        if (!allDay) {
            mEndDateButton.setVisibility(point ? View.GONE : View.VISIBLE);
            mEndTimeButton.setVisibility(point ? View.GONE : View.VISIBLE);
        }
        if (mSpecialDayGroup != null) {
            mSpecialDayGroup.setVisibility(allDay ? View.VISIBLE : View.GONE);
            if (!allDay) {
                mSpecialType = ExtendedProperty.EVENT_TYPE_NONE;
            }
        }
    ...
    // Select the correct toggle buttons for a loaded event without re-running
    // the change side effects (all-day already applied by the caller).
    private void syncTimeModeUi(int mode, int specialType) {
        mUpdatingTimeMode = true;
        mTimeMode = mode;
        if (mTimeModeGroup != null) {
            int id = R.id.btn_mode_period;
            if (mode == TIME_MODE_POINT) {
                id = R.id.btn_mode_point;
            } else if (mode == TIME_MODE_ALL_DAY) {
                id = R.id.btn_mode_allday;
            }
            mTimeModeGroup.check(id);
        }
        mUpdatingTimeMode = false;

        mSpecialType = specialType;
        if (mSpecialDayGroup != null) {
            mSpecialDayGroup.setVisibility(mode == TIME_MODE_ALL_DAY ? View.VISIBLE : View.GONE);
            int sid = R.id.btn_special_none;
            switch (specialType) {
                case ExtendedProperty.EVENT_TYPE_ANNIVERSARY: sid = R.id.btn_special_anniversary; break;
                case ExtendedProperty.EVENT_TYPE_BIRTHDAY: sid = R.id.btn_special_birthday; break;
                case ExtendedProperty.EVENT_TYPE_COUNTDOWN: sid = R.id.btn_special_countdown; break;
                default: sid = R.id.btn_special_none; break;
            }
            mSpecialDayGroup.check(sid);
        }
        if (mode == TIME_MODE_POINT) {
    ...
    public void setModel(CalendarEventModel model) {
        ...
        // ShineFlow: reflect the loaded event into the time-mode selector
        // (time period / time point / all day) and the special-day type.
        int initialTimeMode;
        if (model.mAllDay) {
            initialTimeMode = TIME_MODE_ALL_DAY;
        } else if (model.mStart == model.mEnd) {
            initialTimeMode = TIME_MODE_POINT;
        } else {
    ...
    private void setViewStates(int mode) {
        ...
        if (mode == Utils.MODIFY_UNINITIALIZED || !EditEventHelper.canModifyEvent(mModel)) {
            ...
            // URL is removed from the ShineFlow event editor UI.
            mUrlGroup.setVisibility(View.GONE);
    ...
    public long getCurrentStartMillis() {
        return mStartTime.toMillis();
    }

    public long getCurrentEndMillis() {
        return mEndTime.toMillis();
    }

    public long getCurrentDurationMillis() {
        long duration = mEndTime.toMillis() - mStartTime.toMillis();
        return duration > 0 ? duration : android.text.format.DateUtils.HOUR_IN_MILLIS;
    }
    ...
    public void applySuggestedTimeRange(long startMillis, long endMillis) {
    ...
    public void fillSmartParsedData(
            String title,
            String location,
            long startMillis,
            long durationMillis,
        ...
        if (startMillis > 0) {
            long safeDuration = durationMillis > 0
                    ? durationMillis
                    : android.text.format.DateUtils.HOUR_IN_MILLIS;
            applySuggestedTimeRange(startMillis, startMillis + safeDuration);
        }
    }
