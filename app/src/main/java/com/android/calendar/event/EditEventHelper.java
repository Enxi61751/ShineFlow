...
public class EditEventHelper {
    ...
    public boolean saveEvent(CalendarEventModel model, CalendarEventModel originalModel,
        ...
        // ShineFlow: persist the special all-day event type (birthday / anniversary
        // / countdown) as an extended property.
        {
            Uri extendedPropUri = ExtendedProperty.contentUri(model.mCalendarAccountName, model.mCalendarAccountType);
            String typeValue = ExtendedProperty.valueFromType(model.mSpecialType);
            if (!newEvent) {
                // Remove any previously stored type first.
                ops.add(ContentProviderOperation.newDelete(extendedPropUri)
                        .withSelection(EXTENDED_WHERE_EVENT_NAME,
                                new String[] { Long.toString(model.mId), ExtendedProperty.EVENT_TYPE_NAME })
                        .build());
            }
            if (typeValue != null) {
                values.clear();
                values.put(ExtendedProperties.NAME, ExtendedProperty.EVENT_TYPE_NAME);
                values.put(ExtendedProperties.VALUE, typeValue);
                if (newEvent) {
