...
class MainListPreferences : PreferenceFragmentCompat() {
    ...
    private fun updateCalendarPreferences(screen: PreferenceScreen, allCalendars: List<Calendar>) {
        ...
        // ShineFlow only manages its own local (offline) calendars in Settings.
        // Manufacturer / system / synced calendars keep working in the calendar
        // views but are intentionally hidden from this settings list.
        val offlineAccountName = getString(R.string.offline_account_name)
        val calendars = allCalendars.filter {
            it.isLocal && it.accountName == offlineAccountName
        }
    ...
    private fun addGeneralPreferences(screen: PreferenceScreen) {
        ...
        // ShineFlow has no login / online sync: only local (offline) calendars
        // can be created. The CalDAV (DAVx5) and EteSync entries are removed.
